...
def create_openings_in_revit(doc, walls, wall_data, door_family_symbol, window_family_symbol):
    ...
    for wall, data in zip(walls, wall_data):
        ...
        for child in data.findall('child'):
            ...
            # Lógica unificada de posicionamento
            opening_point = None
            family_symbol = door_family_symbol if structure_type == 'Door' else window_family_symbol
            
            alignment_elem = child.find('alignment')
            alignment = alignment_elem.text.lower() if alignment_elem is not None else 'c'
            half_width = width / 2.0

            # Cálculo do centro horizontal
            if alignment == 'r':
            ...
            # Ajuste vertical específico para janelas
            ...
            for param_name, value in [("Largura", width), ("Altura", height)]:
                param = opening_instance.LookupParameter(param_name)
                if param and not param.IsReadOnly:
                    param.Set(value)

            # Parâmetro específico para janelas
            if structure_type == 'Window' and parapet_val is not None:
                param_parapet = opening_instance.LookupParameter("Altura do peitoril")
                if param_parapet and not param_parapet.IsReadOnly:
                    param_parapet.Set(parapet_val)

            # Aplicação de rotação única
